...
import type { TRadio } from "./Radio.types.js";

export function Radio<T extends TBaseTagMap = "input">(
	{ label, color, size, position = "radio-after", formControl, ...props }: TRadio<T> = { tag: "input" },
): HTMLElementTagNameMap[T] {
	const className = mergeClasses(["radio", color, size, props.className]);
