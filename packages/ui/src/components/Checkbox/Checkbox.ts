...
import type { TCheckbox } from "./Checkbox.types.js";

export function Checkbox<T extends TBaseTagMap = "input">(
	{ label, color, size, position = "checkbox-after", formControl, ...props }: TCheckbox<T> = { tag: "input" },
): HTMLElementTagNameMap[T] {
	const className = mergeClasses(["checkbox", color, size, props.className]);
