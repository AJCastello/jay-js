...
export async function ArticleConventions(slug?: string) {
	slug = slug || (getParams().slug as string);
	const articleData = await ArticleConventions.content.get(slug);
	const collectionData = await ArticleConventions.collection.get();
		...
		children: [
			...
			Box({
				...
				children: [
					...
					Divider(),
					ArticleFooter(collectionData, articleData.articleId, articleData.categoryId, "conventions"),
				],
			}),
...
ArticleConventions.content = useContent<TArticle>({
	fileExt: "md",
	dir: "conventions",
	param: "slug",
});

ArticleConventions.collection = useCollection<TCollection>({
	dir: "conventions",
