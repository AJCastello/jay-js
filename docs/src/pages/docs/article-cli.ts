...
export async function ArticleCli(slug?: string) {
	slug = slug || (getParams().slug as string);
	const articleData = await ArticleCli.content.get(slug);
	const collectionData = await ArticleCli.collection.get();
...
ArticleCli.content = useContent<TArticle>({
	fileExt: "md",
	dir: "cli",
	param: "slug",
});

ArticleCli.collection = useCollection<TCollection>({
	dir: "cli",
