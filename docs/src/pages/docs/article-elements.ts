...
export async function ArticleElements(slug?: string) {
	slug = slug || (getParams().slug as string);
	const articleData = await ArticleElements.content.get(slug);
	const collectionData = await ArticleElements.collection.get();
		...
		children: [
			...
			Box({
				...
				children: [
					...
					Divider(),
					ArticleFooter(collectionData, articleData.articleId, articleData.categoryId, "elements"),
				],
			}),
...
ArticleElements.content = useContent<TArticle>({
	fileExt: "md",
	dir: "elements",
	param: "slug",
});

ArticleElements.collection = useCollection<TCollection>({
	dir: "elements",
