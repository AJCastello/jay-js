...
export async function ElementsSideMenu() {
	const data = await ElementsSideMenu.collection.get();
	const menuItems = groupByCategory(data);
	return Section({
		tag: "aside",
		className: "p-4 w-64 min-w-64",
		children: menuItems.map(item => MenuGroup(item, "elements")),
	});
}

ElementsSideMenu.collection = useCollection<TCollection>({
	dir: "elements",
