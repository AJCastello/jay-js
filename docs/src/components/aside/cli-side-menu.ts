...
export async function CliSideMenu() {
	const data = await CliSideMenu.collection.get();
	const menuItems = groupByCategory(data);
	return Section({
		tag: "aside",
		className: "p-4 w-64 min-w-64",
		children: menuItems.map(item => MenuGroup(item, "cli")),
	});
}

CliSideMenu.collection = useCollection<TCollection>({
	dir: "cli",
