    ...
    children: [
      ...
      Box({
        ...
        children: communityLinks.map(({ title, description, icon, url, linkText }) => {
          return Box({
            className: "border border-base-300 rounded-lg shadow-sm p-6 text-center transition-all duration-200 hover:shadow-md hover:border-primary/50",
            children: [
              Box({
                className: "inline-flex items-center justify-center w-16 h-16 mx-auto rounded-full bg-primary/10 text-primary mb-4",
                children: Icon({
                  className: "text-3xl",
                  icon: icon
                })
      ...
      Box({
        ...
        children: [
          ...
          Typography({
            className: "mb-4",
            children: "Join our weekly live sessions where the team answers your questions and discusses the latest features."
          }),
          Box({
