			...
			Box({
				...
				children: [
					...
					Typography({
						...
						children:
							"Choose exactly what you need. JayJS is split into focused packages that can be used independently or together.",
					}),
				],
			}),
			Box({
				...
				children: packagesList.map(({ name, description, features, icon }) => {
					return Box({
						className:
							"flex flex-col h-full border border-base-300 rounded-lg shadow-sm transition-all duration-200 hover:shadow-md hover:border-primary/50 overflow-hidden",
						children: [
							Box({
