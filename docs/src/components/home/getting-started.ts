...
export function GettingStarted() {
  const steps = [
    ...
    },
    {
      ...
      code: `import { Box, Typography, Button } from "@jay-js/ui";
      ...
      Box({
        ...
        children: steps.map(({ number, title, description, code }) => {
          return Box({
            ...
            children: [
              Box({
                className: "flex-shrink-0 flex items-center justify-center bg-primary text-primary-content w-16 h-16 rounded-full font-bold text-xl",
                children: number
              }),
              Box({
                className: "flex-grow",
